...
class ListUKUpcoming extends ListRecords
{
    ...
    protected function getDistinctCalendars(): array
    {
        ...
        $q = DB::table('class_sessions')
            ...
            ->whereIn(DB::raw($categoryExpr), $this->allowedUkCategories())
            ->orderBy('cal');

        Log::info('[UKUpcoming] Tabs SQL', ['sql' => $q->toSql(), 'bindings' => $q->getBindings()]);
    ...
    protected function getHeaderActions(): array
    {
        ...
        $buttons[] = Action::make('cal_all')
            ...
            ->action(function () {
                ...
                // Clear filters & session to avoid default re-applying
                $this->tableFilters = [];
    ...
    protected function getTableQuery(): Builder
    {
        ...
        $categoryExpr = $this->categoryExpr();
        $calendarExpr = $this->calendarExpr();
        $typeIdExpr = $this->appointmentTypeIdExpr();
        $typeLabelExpr = $this->appointmentTypeLabelExpr();

        $filtersQuery = static::getResource()::getModel()::query()
            ->whereBetween('session_date', [$today, $to])
            ->where(function ($w) {
        ...
        if ($this->selectedCalendar) {
            ...
            $filtersQuery->whereRaw("{$calendarExpr} = ?", [$val]);
        }

        $groupedQuery = (clone $filtersQuery)
            ->select([
                ...
                DB::raw("{$calendarExpr} as calendar_label"),
                DB::raw("MAX({$typeIdExpr}) as appointment_type_id"),
    ...
    private function allowedUkCategories(): array
    {
        return [
            '1. riverside',
            '2. northgate',
